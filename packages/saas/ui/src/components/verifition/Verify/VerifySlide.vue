    ...
    >
      ...
      >
        ...
        >
          ...
          >
            <img :src="'data:image/png;base64,' + blockBackImgBase" style="width: 100%; height: 100%; display: block; -webkit-user-drag: none" />
          </div>
        </div>
      </div>
    </div>
...
import { post } from '@neuton/requests';
import { aesEncrypt } from '../utils/ase';
import { resetSize, createModelId } from '../utils/util';
  ...
  setup(props, context) {
    ...
    function getPictrue() {
      ...
      post('/captcha/get', data)
        .then(d => {
          backImgBase.value = d.originalImageBase64;
          blockBackImgBase.value = d.jigsawImageBase64;
          backToken.value = d.token;
    ...
    function move(e) {
      ...
      if (status.value && isEnd.value === false) {
        ...
        // 拖动后小方块的left值
        moveBlockLeft.value = `${_moveBlockLeft - startLeft.value}px`;
        leftBarWidth.value = `${_moveBlockLeft - startLeft.value}px`;
      }
    }
        ...
        post('captcha/check', { ...data, uniqueCode, action, getType, userId })
          .then(res => {
            ...
            setTimeout(() => {
              ...
              proxy.$parent.$emit('success', { captchaVerification, uniqueCode, action, getType, userId });
            }, 1000);
          })
          .catch(err => {
