  ...
  setup(props, context) {
    ...
    function getPictrue() {
      ...
      post('/captcha/get', data).then(d => {
        pointBackImgBase.value = d.originalImageBase64;
        backToken.value = d.token;
        secretKey.value = d.secretKey;
        poinTextList.value = d.wordList;
    ...
    // 坐标转换函数
    const pointTransfrom = function (pointArr, imageSize) {
        ...
        setTimeout(() => {
          ...
          post('/captcha/check', { ...data, uniqueCode, action, type, userId })
            .then(d => {
              ...
              proxy.$parent.$emit('success', { uniqueCode, captchaVerification });
            })
            .catch(err => {
